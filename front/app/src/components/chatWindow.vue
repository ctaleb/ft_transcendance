  ...
  <div class="chatWindow">
    <div class="header">
      <button class="crossIconButton" @click="closePrivateConv(nickname)">
        <img width="20" height="20" src="https://findicons.com/files/icons/744/juicy_fruit/256/cross.png" alt="" class="crossIconImg" />
      </button>
      <div class="friendInfo">
        <img :src="friendImageUrl" alt="" width="50" height="50" class="miniature" />
        <h4>{{ nickname }}</h4>
      </div>
    </div>
    <div class="messages">
    ...
    <button type="submit" class="sendButton" @click="sendPrivateMessage(nickname)">Send</button>
  </div>
</template>
...
onMounted(() => {
  ...
  fetch("http://" + window.location.hostname + ":3000/api/user/bynickname/" + nickname, {
    headers: {
