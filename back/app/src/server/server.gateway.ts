...
export class ServerGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  gameLoop = (game: Game) => {
    if (game.gameState.frame % 2) game.host.socket.emit('ServerUpdate', this.serverService.sendState(game.gameState, game));
    if (game.gameState.frame % 2) game.client.socket.emit('ServerUpdate', this.serverService.inverseState(game.gameState, game));
